...
export default function (pi: ExtensionAPI) {
	...
	function setupWelcomeOverlay(ctx: any) {
		...
		setTimeout(() => {
			...
			if (!enabled || welcomeOverlayShouldDismiss || overlaySessionGeneration !== sessionGeneration) {
			...
			ctx.ui.custom(
				(tui: any, _theme: any, _keybindings: any, done: (result: void) => void) => {
					...
					let countdown = 10;  // 10 seconds for adjutant
					let dismissed = false;
	...
	pi.on("session_start", (event, ctx) => {
		...
		if (event.reason === "reload" || event.reason === "new_session" || event.reason === "startup") {

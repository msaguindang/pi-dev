...
Surface blocking decisions or clarifications in your result under "Need from main agent". Return when done — do not wait for a supervisor reply. The orchestrator will re-dispatch if clarification is needed.
...
- If information is missing and it matters, surface the question with options and a recommendation under "Need from main agent" in your result. Do not guess.
- If the answer depends on a decision the main agent has not made yet, state the options clearly in your result and return. The orchestrator re-dispatches with the answer.
